...
<body class="bg-gray-100 dark:bg-[#0F1A2E] text-gray-900 dark:text-gray-100 min-h-screen flex">
    <a href="#main-content" class="sr-only focus:not-sr-only focus:absolute focus:z-[100] focus:top-2 focus:left-2 focus:bg-map-blue focus:text-white focus:px-4 focus:py-2 focus:rounded-md focus:text-sm focus:font-bold" data-testid="link-skip-to-content">Skip to main content</a>

    <aside id="sidebar" class="hidden md:flex flex-col w-64 bg-gray-50 dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 shrink-0 h-screen sticky top-0" data-testid="sidebar">
        <div class="p-4 pb-5">
        ...
        <nav class="flex-1 overflow-y-auto px-3" aria-label="Main navigation">
            ...
            <a href="<?= $item['url'] ?>"
               ...
               data-testid="link-nav-<?= strtolower(str_replace(' ', '-', $item['title'])) ?>">
                <i class="icon-<?= $item['icon'] ?> w-4 h-4 <?= $isActive ? 'text-map-teal' : '' ?>"></i>
                <span><?= $item['title'] ?></span>
            </a>
            <?php endforeach; ?>
        </nav>
        <div class="p-4 space-y-2 relative">
            <div class="absolute top-1 left-6 w-1.5 h-1.5 rounded-full bg-map-gold/40"></div>
            <div class="absolute top-3 right-5 w-1 h-1 rounded-full bg-map-gold/30"></div>
            <?php if ($user): ?>
    ...
    <div class="flex flex-col flex-1 min-w-0">
        <header class="flex items-center justify-between gap-3 px-4 py-2 sticky top-0 z-40 relative" style="background: linear-gradient(90deg, #14578F, #1B6EB5, #2384C9)" data-testid="header-main">
            <div class="flex items-center gap-3">
                <button onclick="document.getElementById('mobile-menu').classList.toggle('hidden')" class="md:hidden text-white/90 p-2 min-h-[44px] min-w-[44px] flex items-center justify-center" data-testid="button-mobile-menu" aria-label="Open menu">
                    <img src="/assets/images/logo.png" alt="MapAble" class="h-10 w-auto" data-testid="img-header-logo">
                </button>
                <form action="/care" method="get" class="hidden md:flex items-center gap-2 bg-white/15 backdrop-blur-md border border-white/20 rounded-full px-4 py-1.5 min-w-[240px] lg:min-w-[320px]" data-testid="input-header-search-container">
                    <i class="icon-search w-4 h-4 text-white/70"></i>
                    <input type="search" name="q" placeholder="Search workers, jobs, services..." class="bg-transparent border-none outline-none text-sm text-white placeholder:text-white/50 w-full" data-testid="input-header-search">
                </form>
            </div>
            <div class="flex items-center gap-1">
                <button onclick="toggleTheme()" class="text-white/90 p-2 min-h-[44px] min-w-[44px] flex items-center justify-center" data-testid="button-theme-toggle" aria-label="Toggle theme">
                    <i class="icon-moon w-4 h-4 dark:hidden"></i>
                    <i class="icon-sun w-4 h-4 hidden dark:inline"></i>
                </button>
            </div>
        </header>

        <div id="mobile-menu" class="hidden md:hidden bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 py-2 px-4">
            <?php foreach ($navItems as $item): ?>
            <a href="<?= $item['url'] ?>" class="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-600 dark:text-gray-300 min-h-[44px]">
                <i class="icon-<?= $item['icon'] ?> w-4 h-4"></i>
                <span><?= $item['title'] ?></span>
            </a>
            <?php endforeach; ?>
        ...
        <div class="flex h-[3px] shrink-0" data-testid="accent-tricolor-strip">
